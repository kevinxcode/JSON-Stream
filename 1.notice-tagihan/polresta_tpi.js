(function () {
  const now = new Date();
  const expiredDate = new Date(now.getFullYear(), 10, 20); // 6 = Juli (0-based)
  const showNoticeDate = new Date(expiredDate);
  showNoticeDate.setDate(expiredDate.getDate() - 40);

  if (now >= showNoticeDate && now <= expiredDate) {
        ...
        <div style="display: flex; align-items: center; margin-right: 10px;">
          ...
          <div style="color: #FFFFFF; font-size: 14px;">
            <b style="padding-right: 5px;">
              Website Akan Expired pada tanggal 20 Oktober <span id="expired-year"></span>
            </b>
    ...
    document.addEventListener("DOMContentLoaded", function () {
      ...
      document.getElementById('expired-year').textContent = expiredDate.getFullYear();
